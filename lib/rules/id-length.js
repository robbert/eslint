    ...
    create: function(context) {
        ...
        return {
            Identifier: function(node) {
                ...
                var delta = isLong ? name.length - maxLength : minLength - name.length;
                ...
                if (isValidExpression && (isValidExpression === true || isValidExpression(parent, node))) {
                    context.report(
                        node,
                        isShort ?
                            "Identifier name '{{name}}' is too short, length needs to be increased by {{delta}}." :
                            "Identifier name '{{name}}' is too long, exceeding the maximum {{max}} by {{delta}}.",
                        { name: name, min: minLength, max: maxLength, delta: delta }
                    );
                }
            }
        };
