...
module.exports = {
    ...
    create: function(context) {
         ...
         *
         * Always use double quotes since it is not possible to access the settings for the `quotes` rule.
         * #5297 mentions eslint 3.0.0 might have provide access to these settings, and fix it accordingly.
         * It seems more desirable to import an actual `function quote(str, context)` from `quotes.js`.
         * Another solution might be to perform `quotes` as a compile pass after `quote-props`.
         *
         * @param {string} str String that will be quoted
        ...
        function checkOmittedQuotes(node) {
            ...
            if (!node.method && !node.computed && !node.shorthand && !(key.type === "Literal" && typeof key.value === "string")) {
                context.report(node, MESSAGE_UNQUOTED, {
                    ...
                    fix: function(fixer) {
                        fixer.replaceText(key, quote(key.value));
                    }
                });
            }
        }
